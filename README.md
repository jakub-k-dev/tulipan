...
**Lighthouse — cache & image size:** GitHub Pages does not send long-lived `Cache-Control` headers, so “Use efficient cache lifetimes” will show 10m TTL until you add **Cloudflare** in front. See **`docs/notes/cache-headers-github-pages.md`** (in repo root `web/`) for step-by-step Cloudflare Transform Rules. Hero uses AVIF/JPEG and optimized compression; sponsor logos use WebP where applicable.
